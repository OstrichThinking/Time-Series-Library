...
from data_provider.data_format import read_data, convert_jsonl_to_sample_list
...
class VitalDBLoader_JSONL(Dataset):
    ...
    def __read_data__(self):
        ...
        # 根据 falg 分割数据集
        if self.flag == 'train':
            case_subset = read_data(os.path.join(self.root_path, str(self.data_path)), self.flag)
        elif self.flag == 'test':
            case_subset = read_data(os.path.join(self.root_path, str(self.data_path)), self.flag)
        else:
            case_subset = read_data(os.path.join(self.root_path, str(self.data_path)), self.flag)

        self.__process_data(case_subset)


    def __process_data(self, case_subset):
        
        sample_list = convert_jsonl_to_sample_list(case_subset, self.flag,
                                                   self.seq_len, self.pred_len, self.s_win, self.stime, self.exp_stime,
                                                   self.static_features, self.dynamic_features)
        
        if self.args.augment_method and self.flag == 'train':
            print("Augmenting data...")
            if self.args.augment_method == 'oversample_minority':
                self.data_augmentor = OversampleMinorityDataBalancer(self.args, X=sample_list, method=self.args.augment_method)
            sample_list, _ = self.data_augmentor.augment(sample_list, )
            
        self.__scale_data(sample_list)
        
    
    def __scale_data(self, sample_list):
